    ...
    def draw(self, context):
        ...
        if not utils.is_installation_valid():
            ...
            utils.label_multiline(
                layout, text="Enable AI Render in this scene to start using Stable Diffusion", alignment="CENTER")
...
class AIR_PT_setup(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not AIR_PT_setup.is_api_key_valid(context) and utils.sd_backend(context) == "dreamstudio":
            ...
            utils.label_multiline(
                layout, text="Setup is quick and easy. No downloads or installation. Just register for a DreamStudio API Key.", icon="INFO", width=width_guess)

            row = layout.row()
            col = row.column()
            col.operator(operators.AIR_OT_setup_instructions_popup.bl_idname,
                         text="Instructions", icon="HELP")

            row = layout.row()
            row.operator("wm.url_open", text="Watch Full Tutorial",
                         icon="URL").url = config.VIDEO_TUTORIAL_URL
            ...
            row = layout.row()
            row.prop(utils.get_addon_preferences(
                context), "dream_studio_api_key")
        ...
        # show the image dimension help if the dimensions are invalid or too large
            ...
            col = row.column()
            col.operator(
                operators.AIR_OT_show_other_dimension_options.bl_idname, text="Other")

            if utils.get_active_backend().supports_upscaling() and props.do_upscale_automatically:
                layout.separator()
                box = layout.box()
                utils.label_multiline(
                    box, text=f"Final image will be upscaled {round(props.upscale_factor)}x larger than these initial dimensions.", width=width_guess-20)

        # else, show the ready / getting started message and disable and change image size buttons
        else:
            utils.label_multiline(
                layout, text="You're ready to start rendering!", width=width_guess, alignment="CENTER")
            row = layout.row()
            row.operator("wm.url_open", text="Help Getting Started",
                         icon="URL").url = config.VIDEO_TUTORIAL_URL

            row = layout.row(align=True)
            row.operator(
                operators.AIR_OT_show_other_dimension_options.bl_idname, text="Change Image Size")
            row.separator()
            row.operator(operators.AIR_OT_disable.bl_idname,
                         text="Disable AI Render")
...
class AIR_PT_prompt(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if props.use_preset:
            row = box.row()
            row.template_icon_view(props, "preset_style",
                                   show_labels=True, scale_popup=7.7)
            ...
            col = row.column()
            col.operator(operators.AIR_OT_copy_preset_text.bl_idname,
                         text="", icon="COPYDOWN")
...
class AIR_PT_controlnet(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not props.controlnet_close_help:
            ...
            split.prop(props, "controlnet_close_help",
            ...
            row = box.row()
            row.operator("wm.url_open", text="Learn More",
                         icon="URL").url = config.HELP_WITH_CONTROLNET_URL
        ...
        if not props.controlnet_available_models:
            row = layout.row()
            row.operator(operators.AIR_OT_automatic1111_load_controlnet_models_and_modules.bl_idname,
                         text="Load Models from Automatic1111", icon="FILE_REFRESH")
        else:
            # Heads up box
            ...
            split = row.split(align=True)
            split.operator(
                operators.AIR_OT_automatic1111_load_controlnet_modules.bl_idname, text="", icon="FILE_REFRESH")
            ...
            split = row.split(align=True)
            split.operator(
                operators.AIR_OT_automatic1111_load_controlnet_models.bl_idname, text="", icon="FILE_REFRESH")
...
class AIR_PT_operation(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.enabled = 'Render Result' in bpy.data.images and bpy.data.images[
            'Render Result'].has_data
        row.operator(operators.AIR_OT_generate_new_image_from_render.bl_idname)

        row = layout.row()
        row.enabled = props.last_generated_image_filename != ""
        row.operator(
            operators.AIR_OT_generate_new_image_from_last_sd_image.bl_idname)
...
class AIR_PT_upscale(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not AIR_PT_upscale.is_upscaler_model_list_loaded(context):
            utils.label_multiline(
                layout, text="To get started upscaling, load the available upscaler models", icon="ERROR", width=width_guess)
            layout.operator(operators.AIR_OT_automatic1111_load_upscaler_models.bl_idname,
                            text="Load Upscaler Models", icon="FILE_REFRESH")
            return
        ...
        if not AIR_PT_upscale.are_upscaled_dimensions_small_enough(context):
            utils.label_multiline(
                layout, text="Upscaled dimensions are too large. Please decrease the scale factor.", icon="ERROR", width=width_guess)
        ...
        if AIR_PT_upscale.does_backend_support_reloading_upscaler_model_list(context):
            row = layout.row()
            row.operator(operators.AIR_OT_automatic1111_load_upscaler_models.bl_idname,
                         text="Reload Upscaler Models", icon="FILE_REFRESH")

        # show button to manually upscale
        row = layout.row()
        row.enabled = props.last_generated_image_filename != ""
        row.operator(operators.AIR_OT_upscale_last_sd_image.bl_idname,
...
class AIR_PT_animation(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator(operators.AIR_OT_render_animation.bl_idname,
                     icon="RENDER_ANIMATION", text=render_animation_text)
        ...
        if round(props.image_similarity, 2) < 0.7 and not props.close_animation_tips:
            ...
            split = row.split(align=True)
            split.prop(props, "close_animation_tips",
                       text="", icon="X", emboss=False)

            utils.label_multiline(
                box, text="For more stable animations, consider using ControlNet (locally) or increasing \"Image Similarity\" to at least 0.7", width=width_guess)

            row = box.row()
            row.operator("wm.url_open", text="Get Animation Tips",
                         icon="URL").url = config.ANIMATION_TIPS_URL
